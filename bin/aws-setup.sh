...
ecs-cli configure \
  ...
  --default-launch-type $AWS_ECS_DEFAULT_CLUSTER_LAUNCH_TYPE \
  --config-name $AWS_ECS_CLUSTER_CONFIG

ecs-cli configure profile \
  --access-key $AWS_ACCESS_KEY_ID \
  --secret-key $AWS_SECRET_ACCESS_KEY \
  --profile-name $AWS_ECS_CLUSTER_CONFIG

ecs-cli up \
  --verbose \
  --cluster-config $AWS_ECS_CLUSTER_CONFIG \
  --keypair $AWS_KEY_PAIR \
  --size $AWS_ECS_INSTANCE_SIZE \
  --instance-type $AWS_ECS_INSTANCE_TYPE \
  --image-id $AWS_ECS_INSTANCE_IMAGE_ID \
  --launch-type $AWS_ECS_DEFAULT_CLUSTER_LAUNCH_TYPE \
  --vpc $AWS_VPC \
  --subnets $AWS_SUBNETS \
  --security-group $AWS_SECURITY_GROUP \
  --instance-role $AWS_INSTANCE_ROLE
